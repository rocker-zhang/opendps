...
class PriorityTieredPRSBrain:
    ...
    def __init__(
        ...
        **prs_kwargs,
        ...
        if bad:
            raise ValueError(
                f"unknown priority tier(s) {sorted(bad)}; valid: {sorted(TIER_WEIGHTS)}"
            )
        self._topology = topology
        self._prs = PRSBrain(topology, **prs_kwargs)
        self._tiers_lock = threading.Lock()
        self._tiers: dict[int, str] = {}
        self.set_tiers(gpu_priority_tiers)
        self._threshold = contention_threshold
        self._min_cap_w = min_cap_w

    def set_tiers(self, gpu_priority_tiers: Mapping[int, str]) -> None:
        """Atomically replace the GPU-to-priority-tier mapping."""
        tiers = dict(gpu_priority_tiers)
        bad = {tier for tier in tiers.values() if tier not in TIER_WEIGHTS}
        if bad:
            raise ValueError(
                f"unknown priority tier(s) {sorted(bad)}; valid: {sorted(TIER_WEIGHTS)}"
            )
    ...
    def decide(self, domain_name: str, state: DomainState) -> BrainDecision:
        ...
        weights = {
            g: state.gpu_draws.get(g, 0.0) * TIER_WEIGHTS[tiers.get(g, DEFAULT_TIER)]
            for g in contended
        }
